 ...
 *   ganó, el recurso relacionado es su lista de compras (no el detalle puntual); para el
 *   rematador (misma `type`, notificación separada), es la venta adjudicada puntual.
...
export function notificationHref(notification: Notification, role: UserRole | undefined): string | null {
  ...
  if (resourceType === 'postauction_case' && resourceId) {
    if (type === 'postauction.case_created' && role !== 'rematador') {
      return '/mis-compras';
    }
    return role === 'rematador' ? `/ventas-adjudicadas/${resourceId}` : `/mis-compras/${resourceId}`;
  }
